...
async def start_session():
    """Create a fresh conversation session and return session_id."""
    session_id = str(uuid.uuid4())
    state = ConversationState()
    # Ensure the structure matches ConversationState expectations
    state["messages"] = []
    state["active_agent"] = "user_input"
    state["end_conversation"] = False
    state["clinical_engaged"] = False
    state["user_message"] = ""
...
async def chat_endpoint(input_data: MessageInput):
    ...
    if current_agent == "user_input":
        ...
        state = receptionist_node(state)
        # Only collect new messages generated by the agent
        new_messages = state["messages"][prev_msg_count:]
        for msg in new_messages:
            # Check if it is the medical concern handoff message
            if msg["content"].startswith("That sounds like a medical concern") and state.get("receptionist_handoff_shown"):
                # Skip appending if already shown
                continue
            ...
            # Mark that the handoff message has been sent
            if msg["content"].startswith("That sounds like a medical concern"):
                state["receptionist_handoff_shown"] = True
